...
## Recent Changes
- **2026-04-04**: Voice input implemented — `expo-av` Audio.Recording records m4a audio; mic button added to AIChatScreen input bar (🎙/⬛/spinner states with pulsing animation when recording); `POST /api/transcribe` server endpoint decodes base64 audio and calls Whisper-1 via globalai.vip; transcribed text appended to input field; iOS NSMicrophoneUsageDescription added to app.json; i18n keys `micPermissionTitle` + `micPermissionMsg` added to en.json/ru.json.
- **2026-04-03**: Offline detection — animated banner, retry logic, canSend blocked offline via `@react-native-community/netinfo`.
